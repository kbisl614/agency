...
**Architecture:** Hybrid model with independent workflows (fire on trigger) + orchestrated hub (routes ambiguous input). All workflows call FastAPI backend for Claude decisions, then execute actions (SMS, Airtable, CRM writes).

**Tech Stack:** n8n, FastAPI backend, Claude API (claude-sonnet-4-20250514), Jobber webhooks, Twilio SMS, Airtable → Supabase
...
Jobber webhooks & Twilio SMS
   ...
   Airtable (Leads, Actions, Waitlist tables)
        ↓
...
**Trigger:** Inbound SMS to Twilio webhook → posts to n8n webhook URL
...
- Lookup in Airtable: does this phone exist in Leads table?
- Output: `{ phone, message, timestamp, is_returning_customer }`

**3. Call Claude API** (HTTP POST to FastAPI)
...
**4. Create Lead Record** (Airtable node)
...
| Scenario | Handling |
|----------|----------|
| Twilio webhook malformed | Log error to Actions, STOP |
| Claude API timeout | Log error to Actions, STOP, don't send SMS |
| Airtable write fails | Log error to Actions, STOP, don't send SMS |
| Twilio SMS send fails | Log to Actions with status "sms_failed" |
...
- **Key nodes:** Twilio webhook → Extract phone + reply → Query lead by phone (most recent active) → Claude classification → Route to action → Send SMS confirmation → Log action
...
- **Key nodes:** Scheduled trigger → Query Waitlist → Claude decision → Update Airtable → Log action
...
| Waitlist Manager | 6 AM daily | Waitlist table | Prioritize + age out | Update Airtable |
...
- **TCPA (US texting law):** Customer must explicitly opt-in for marketing (seasonal outreach). Keep consent records + timestamp in Airtable.
...
Webhook receives: { type, customer_phone, message/job_data }
    ↓
Query Airtable: Does this customer exist? (match by phone)
    ↓
...
## Section 8: Database Schema — Airtable (MVP) → Supabase (Phase 2)
...
- Airtable API rate limits (5 req/sec) will hit when you have 100+ contractors
- Supabase is PostgreSQL = unlimited scale
- Same data structure, easier joins

**Migration approach:**
1. Create identical Supabase tables (same schema as Airtable)
2. Dual-write for 1 week (n8n writes to both Airtable + Supabase)
3. Verify data consistency
4. Switch n8n read queries to Supabase
5. Keep Airtable for 2 weeks as backup, then decommission

**No code changes needed in FastAPI** — just change database connection string.
...
- FastAPI endpoints: No change (already CRM-agnostic)
...
- Same pattern: new n8n workflows, same FastAPI logic
