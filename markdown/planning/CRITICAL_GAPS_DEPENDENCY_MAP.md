...
**Actions Required:**
1. **Supabase `actions` table:** Add these columns
   - `revenue_impact` (numeric, required on every record)
   - `confidence_score` (numeric 0–100)
   - `confidence_tier` (text: auto_execute | auto_with_flag | escalated)
   - `action_type` (text: lead_capture | missed_call | cancellation_fill | invoice_sent | sms_response | etc.)
...
3. **Supabase `leads` table:** Add these columns
   - `conversation_id` (uuid, required for Workflow 9 threading)
   - `thread_history` (jsonb, array of previous messages)
   - `revenue_impact` (numeric, reference to associated action)

4. **New `error_log` table in Supabase:**
   ```sql
...
```
SignalWire Missed Call Webhook
         ↓
    Extract phone
         ↓
...
**Build Time:** 4 hours (SignalWire webhook + Claude integration + SMS)
...
**Prerequisites:** Gap #3 (conversation_id + thread_history columns must exist in Supabase)

```
Inbound SMS from customer
         ↓
Query Supabase leads for conversation_id
         ↓
...
**Build Time:** 6 hours (Supabase query + Claude context building + response chaining)
**Database Prerequisite:** conversation_id + thread_history columns MUST exist before building
...
2. **BEFORE CODE:** Run `docs/SUPABASE_SCHEMA.sql` with Gap #3 columns (revenue_impact, confidence_score, thread history)
...
1. **BEFORE CODE:** Verify conversation_id + thread_history columns exist in Supabase leads table
...
- [ ] Gap #3 Supabase schema finalized (revenue_impact, confidence_score, thread_history columns added)
...
- [ ] Database migrations run before code begins

**Do not start building Week 1 until all checkboxes are complete.**

---

*Last updated: 2026-03-15 | Ready for implementation*
