> ⚠️ **ARCHIVED PLAN** — This plan was written for the original FastAPI + Airtable architecture, which has been archived. The current architecture uses n8n + Supabase + SignalWire with no separate backend service. Code snippets in this file reference backend/ directory and Python services that no longer exist. Reference `docs/WORKFLOW_BUILD_COMPLETE.md` for current state.

# Phase 1 Implementation Plan — Tier 1 Workflows (Detailed & Thorough)

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build three production-ready Tier 1 workflows (After-hours lead capture + Missed call recovery + Audit trail) with n8n backend, n8n orchestration, and Mission Control dashboard. First client deliverable.

**Architecture:** n8n backend handles Claude decisions + Supabase writes. n8n workflows orchestrate SignalWire SMS triggers. Mission Control dashboard polls Supabase every 10 seconds showing real-time action log + ROI metrics. All actions logged for audit trail.

**Tech Stack:** n8n (Python 3.11+), Claude API (claude-sonnet-4-20250514), n8n (self-hosted or Cloud), Supabase, SignalWire, Next.js 14+, PostgreSQL (optional, Phase 2)

**Success Criteria:**
- ✅ After-hours SMS test works within 2 seconds, logged to Supabase
- ✅ Missed call SMS test works within 60 seconds, logged to Supabase
- ✅ Mission Control dashboard shows action log + revenue metrics in real-time
- ✅ All 3 workflows have >95% test coverage
- ✅ n8n deployed (Railway or Render)
...
│   │   ├── airtable_service.py           ← Supabase CRUD operations
│   │   └── twilio_service.py             ← SMS sending via SignalWire
│   └── tests/
│       ├── __init__.py
│       ├── test_claude_service.py        ← Claude qualification tests
│       ├── test_airtable_service.py      ← Supabase operation tests
...
## Chunk 1: n8n Backend Setup & Services

### Task 1: Initialize n8n project and dependencies
...
git commit -m "chore: initialize n8n project dependencies"
```
...
- [ ] **Step 1: Write test for Supabase operations**

```python
# File: backend/tests/test_airtable_service.py
import pytest
from backend.services.airtable_service import SupabaseService
from unittest.mock import patch, MagicMock
...
def test_create_lead():
    """Test creating a lead record in Supabase"""
...
- [ ] **Step 3: Write Supabase service implementation**

```python
# File: backend/services/airtable_service.py
from airtable import Supabase
from backend.config import settings
...
class SupabaseService:
    ...
    def create_lead(
        ...
        """Create a new lead record in Supabase"""
        fields = {
            "Phone": phone,
            "Message": message,
    ...
    def log_action(
        ...
        if lead_id:
            fields["Lead ID"] = [lead_id]  # Supabase expects array for linked records
        if error_message:
...
git commit -m "feat: implement Supabase service for CRUD operations and audit trail logging"
```

---

### Task 5: Create SignalWire service
...
import pytest
from backend.services.twilio_service import SignalWireService
from unittest.mock import patch, MagicMock
...
Expected: FAIL - "SignalWireService not found"

- [ ] **Step 3: Write SignalWire service implementation**
...
class SignalWireService:
    ...
    def send_sms(self, to_phone: str, message_text: str) -> Dict[str, Any]:
        ...
        {
            "success": bool,
            "sms_sid": str (SignalWire message ID),
            "error": str (if failed)
        }
        """
        try:
...
git commit -m "feat: implement SignalWire service for SMS sending"
```

---

## Chunk 2: n8n Routes & Endpoints
...
airtable_service = SupabaseService()
twilio_service = SignalWireService()
...
git commit -m "feat: initialize n8n app with CORS and route registration"
```

---

## Chunk 3: Supabase Schema Setup

### Task 9: Create Supabase tables schema

**Files:**
- Create: `docs/AIRTABLE_SCHEMA.md` (documentation)

- [ ] **Step 1: Document Supabase schema requirements**

```markdown
# File: docs/AIRTABLE_SCHEMA.md

## Supabase Base Setup for Phase 1
...
| SMS SID | Single line text | No | SignalWire message ID (if SMS sent) |
...
1. Create a new Supabase Base named "HVAC AI Operations - Phase 1"
2. Create Leads table with fields above
3. Create Actions table with fields above
4. Set Phone field as unique (Supabase will prevent duplicates)
5. Set up views:
...
- [ ] **Step 2: Create Supabase tables via API script**

```python
# File: backend/scripts/setup_airtable.py
#!/usr/bin/env python3
"""
Script to set up Supabase base and tables for Phase 1.
...
def setup_leads_table():
    ...
    airtable = Supabase(AIRTABLE_BASE_ID, AIRTABLE_API_KEY)

    # Note: Supabase API doesn't support creating tables directly.
    # You'll need to create tables in Supabase UI first, then configure fields.

    print("✓ Leads table: Create manually in Supabase UI with these fields:")
...
def setup_actions_table():
    ...
    print("✓ Actions table: Create manually in Supabase UI with these fields:")
...
**Trigger:** SignalWire webhook (inbound SMS)

**Nodes:**
1. **SignalWire Webhook** - Receive SMS from SignalWire
2. **Extract Data** - Parse phone, message, timestamp
3. **HTTP Request** - Call n8n POST /leads/qualify
...
4. In SignalWire, set webhook for inbound SMS to n8n URL
5. Add HTTP node pointing to n8n /leads/qualify
6. Map fields: phone, message, is_new_customer
7. Add conditional based on response.confidence
8. Configure SMS node (SignalWire) for high confidence path
9. Add Supabase nodes (if using native n8n integration)
10. Deploy and test
...
3. **HTTP Request** - Call n8n POST /webhooks/missed-call
4. **Conditional** - Check if should_send?
5. **If Should Send:** Send SMS
6. **Log Action** - Log to Supabase (via n8n)
...
5. Add HTTP node pointing to n8n /webhooks/missed-call
...
```
FASTAPI_URL=https://api.example.com  # n8n backend URL
TWILIO_PHONE_NUMBER=+1234567890
...
1. Send test SMS to SignalWire number (after-hours workflow)
2. Trigger test missed call (missed-call workflow)
3. Verify Supabase has new records
4. Check action log in Supabase Actions table
5. Verify SMS was sent (check SignalWire logs)
```
...
```json
{
  ...
  "content": {
    ...
    "nodes": [
      {
        "parameters": {
          "name": "SignalWire Webhook - After Hours Lead",
...
- [ ] **Step 1: Create useActionLog hook for polling Supabase**
...
export function useActionLog(pollingInterval: number = 10000) {
  ...
  const fetchActions = useCallback(async () => {
    try {
      setLoading(true);
      // In Phase 1, we'll query Supabase directly via API
      // Later, we'll add a n8n endpoint for this
...
export async function GET(request: NextRequest) {
  try {
    // In Phase 1, query Supabase directly
    // Later, this will call n8n backend
    ...
    if (!actionsRes.ok) {
      throw new Error(`Supabase API error: ${actionsRes.status}`);
    }
...
1. Send test SMS to SignalWire number after 7pm (or set up test mode)
2. Verify SMS response received within 2 seconds
3. Check Supabase Leads table - new lead should exist
4. Check Supabase Actions table - "sms_sent" action logged
...
3. Check Supabase - lead and action logged
...
- ✅ `/health` - Health check
- ✅ All services: Claude, Supabase, SignalWire
...
- ✅ Supabase Actions table (audit trail)
...
- Integrated with n8n backend
- Ready for Jobber/SignalWire webhooks
